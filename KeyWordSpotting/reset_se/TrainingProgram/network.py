...
from pooling import AttentiveStatsPool, TemporalAveragePooling
from pooling import SelfAttentivePooling, TemporalStatisticsPooling
...
class SELayer(nn.Module):
    ...
    def forward(self, x):
        b, c, _, _ = x.size()
        #print("roger dump x.size:{0}".format(x.size()))
        #y = self.avg_pool(x).view(b, c)
        y = F.avg_pool2d(x, kernel_size=x.size()[2:]).view(b, c)
        #print("roger dump y.size:{0}".format(y.size()))
...
class KWS(nn.Module):
    def __init__(self, num_class, input_size, layers=[2, 2, 4, 2], num_filters=[4, 8, 16, 32], embd_dim=64,
        ...
        if pooling_type == "ASP":
            self.pooling = AttentiveStatsPool(cat_channels, 82)
            ...
            self.pooling = SelfAttentivePooling(cat_channels, 32)
...
if __name__ == "__main__":
    import torch
    net = KWS(3, pooling_type='TAP', input_size=40)
    net.eval()
    x = torch.randn(1, 160, 40).float()
    y = net(x)
    print(y.shape)
